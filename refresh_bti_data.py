...
def build_ldp_data(orders, payments_rows=None, payments_csv_path=None):
    ...
    for r in orders:
        ...
        rows_out.append([
            ...
            str(r.get("REFUND_CREDIT_DATE","") or "")[:10] if cncl in ("Cancelled","Downgrade","Upgrade") else "",  # [36] status change date YYYY-MM-DD
            get_rd_days(r.get("REFUND_CREDIT_DATE",""), r.get("DATE","")) if cncl in ("Cancelled","Downgrade","Upgrade") else -1,  # [37] days purchase→status change
        ])
