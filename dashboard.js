...
function getMergedMonthly(partners){
  var r=getRange(),byM={};
  partners.forEach(function(p){var pm=(D.PM&&D.PM[p])||{};Object.keys(pm).forEach(function(m){if(m<r.df||m>r.dt)return;if(!byM[m])byM[m]=[0,0,0,0,0,0,0,0];var v=pm[m];for(var i=0;i<8;i++)byM[m][i]+=(v[i]||0);});});
  return byM;
}

function getMonthlyBuckets(){
  var r=getRange(),pcat=document.getElementById("fPcat").value;
  if(selP.size>0)return Object.values(getMergedMonthly(Array.from(selP)));
  if(pcat){var pm=D.PCM[pcat]||{};return Object.keys(pm).filter(function(m){return m>=r.df&&m<=r.dt;}).map(function(m){return pm[m];});}
  return Object.keys(D.M).filter(function(m){return m>=r.df&&m<=r.dt;}).map(function(m){return D.M[m];});
}

function getTimeSeries(){
  var r=getRange(),pcat=document.getElementById("fPcat").value;
  if(selP.size>0){var bm=getMergedMonthly(Array.from(selP));return Object.keys(bm).sort().map(function(m){return{m:m,b:bm[m]};});}
  if(pcat){var pm=D.PCM[pcat]||{};return Object.keys(pm).filter(function(m){return m>=r.df&&m<=r.dt;}).sort().map(function(m){return{m:m,b:pm[m]};});}
  return Object.keys(D.M).filter(function(m){return m>=r.df&&m<=r.dt;}).sort().map(function(m){return{m:m,b:D.M[m]};});
}
...
function render(){
  destroyCharts();
  var bkts=getMonthlyBuckets(),tot=sumArr(bkts);
  ...
  var pcat=document.getElementById("fPcat").value;
  ...
  treePath=[];buildDecompTree(T,AC,IN,C,E,U,Dv,ts);

  var skuArr=Object.keys(D.S).map(function(sku){var v=D.S[sku];return{sku:sku,T:v[Ti],C:v[Ci],E:v[Ei],U:v[Ui],D:v[Di],AC:v[Ai],IN:v[Ii],LR:v[CRi],sale:Math.max(0,v[Ti]-v[Ci]-v[Ei]-v[Ui]-v[Di]),rate:v[Ti]>0?((v[Ci]+v[Ei])/v[Ti]*100):0};}).sort(function(a,b){return b.C-a.C;});
  var top15=skuArr.slice(0,15),bh=Math.max(320,top15.length*38);
  ...
  var pKeys=Object.keys(D.PC).sort(function(a,b){return(D.PC[b].cancelled||0)-(D.PC[a].cancelled||0);});
  charts.pcat=new Chart(document.getElementById("pcatChart"),{type:"doughnut",data:{labels:pKeys,datasets:[{data:pKeys.map(function(k){return(D.PC[k].cancelled||0)+(D.PC[k].entry_error||0);}),backgroundColor:["#388bfd","#f85149","#3fb950","#e3b341","#bc8cff"],borderWidth:0,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:"62%",plugins:{legend:{position:"right",labels:{color:"#8b949e",font:{size:11},boxWidth:10,padding:8}}}}});
  var rdK=["0","15","30","45","60","61"],rdL=["Same day","<=15d","<=30d","<=45d","<=60d","61+d"];
  charts.rd=new Chart(document.getElementById("rdChart"),{type:"bar",data:{labels:rdL,datasets:[{data:rdK.map(function(k){return D.RD[k]||0;}),backgroundColor:"rgba(56,139,253,0.75)",borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:"#8b949e",font:{size:10}},grid:{color:"#21262d44"}},y:{ticks:{color:"#8b949e",font:{size:10}},grid:{color:"#21262d44"}}}}});
...
function buildDecompTree(T,AC,IN,C,E,U,Dv,ts){
  var rmTop=Object.entries(D.RM).sort(function(a,b){return b[1]-a[1];}).slice(0,10);
  ...
  var rdItems=Object.entries(D.RD).filter(function(e){return rdMap[e[0]];}).sort(function(a,b){return +a[0]-+b[0];});
  var rdTot=rdItems.reduce(function(s,e){return s+e[1];},0)||1;
  var myMap={};ts.forEach(function(x){var p=x.m.split("-");var lbl=new Date(parseInt(p[0]),parseInt(p[1])-1).toLocaleString("default",{month:"short",year:"numeric"});myMap[lbl]=(myMap[lbl]||0)+x.b[Ti];});
  var myTop=Object.entries(myMap).sort(function(a,b){return b[1]-a[1];}).slice(0,10);
  var sale=Math.max(0,T-C-E-U-Dv);
  currentLevels=[
    {title:"Total Units",items:[{l:"Total Units",v:T,c:"#388bfd",pct:"100"}]},
    {title:"Active Status",items:[{l:"Active",v:AC,c:"#58a6ff",pct:T>0?(AC/T*100).toFixed(1):"0"},{l:"Inactive",v:IN,c:"#f85149",pct:T>0?(IN/T*100).toFixed(1):"0"}]},
    {title:"Month Year",items:myTop.map(function(e){return{l:e[0],v:e[1],c:"#388bfd",pct:T>0?(e[1]/T*100).toFixed(1):"0"};})},
    {title:"Refund Month",items:rmTop.map(function(e){return{l:e[0],v:e[1],c:"#f85149",pct:(e[1]/rmTot*100).toFixed(1)};})},
    {title:"CNCL Status",items:[{l:"Sale",v:sale,c:"#39d353",pct:T>0?(sale/T*100).toFixed(1):"0"},{l:"Cancelled",v:C,c:"#f85149",pct:T>0?(C/T*100).toFixed(1):"0"},{l:"Entry Error",v:E,c:"#e3b341",pct:T>0?(E/T*100).toFixed(1):"0"},{l:"Upgrade",v:U,c:"#3fb950",pct:T>0?(U/T*100).toFixed(1):"0"},{l:"Downgrade",v:Dv,c:"#bc8cff",pct:T>0?(Dv/T*100).toFixed(1):"0"}].filter(function(x){return x.v>0;})},
...
function renderDecompLevels(){
  ...
  var tree=document.getElementById("decompTree");
  tree.innerHTML="";
  for(var li=0;li<=showUpTo;li++){
    ...
    if(li>0){var conn=document.createElement("div");conn.style.cssText="width:28px;align-self:stretch;display:flex;align-items:center;flex-shrink:0";conn.innerHTML='<div style="width:100%;height:1px;background:'+(treePath.length>=li?"#388bfd":"#30363d")+'"></div>';tree.appendChild(conn);}
    var col=document.createElement("div");col.style.cssText="display:flex;flex-direction:column;flex-shrink:0;width:160px";
    var hdr=document.createElement("div");hdr.style.cssText="font-size:10px;font-weight:600;color:#8b949e;text-transform:uppercase;letter-spacing:.7px;text-align:center;padding:5px 10px 8px;border-bottom:1px solid #30363d;margin-bottom:6px";hdr.textContent=level.title;col.appendChild(hdr);
    var wrap=document.createElement("div");wrap.style.cssText="display:flex;flex-direction:column;gap:5px;max-height:400px;overflow-y:auto";
    ...
    for(var idx=0;idx<level.items.length;idx++){
      (function(item,capturedIdx,capturedLi){
        ...
        node.innerHTML='<div style="font-size:10px;color:#8b949e;margin-bottom:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+item.l+'</div><div style="font-size:20px;font-weight:700;color:'+item.c+';margin-bottom:5px">'+Number(item.v).toLocaleString()+'</div><div style="height:3px;background:#21262d;border-radius:2px;overflow:hidden;margin-bottom:4px"><div style="height:100%;width:'+bw+'%;background:'+item.c+';border-radius:2px"></div></div><div style="font-size:10px;font-weight:500;color:'+item.c+'">'+item.pct+'% of total</div>'+(canClick?'<div style="font-size:9px;color:#388bfd88;margin-top:3px">click to drill down</div>':"");
        if(canClick){
...
function buildDecompBC(){
  ...
  function addSep(){var s=document.createElement("span");s.style.cssText="color:#30363d;font-size:12px";s.textContent=">";bc.appendChild(s);}
  crumb("Total Units",function(){treePath=[];renderDecompLevels();buildDecompBC();},treePath.length===0);
  treePath.forEach(function(selIdx,i){addSep();var item=currentLevels[i+1]&&currentLevels[i+1].items[selIdx];if(item){(function(ci){crumb(item.l,function(){treePath=treePath.slice(0,ci+1);renderDecompLevels();buildDecompBC();},i===treePath.length-1);})(i);}});
