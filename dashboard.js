...
function render(){
  ...
  // Partner donut - date aware using pcat monthly
  var r2=getRange();
  var pcatTotals={};
  Object.keys(D.PCMSKU||{}).forEach(function(pc){
    ...
    Object.keys(pm).filter(function(m){return m>=r2.df&&m<=r2.dt;}).forEach(function(m){
      Object.values(pm[m]).forEach(function(v){cncl+=v[Ci]||0;ee+=v[Ei]||0;});
    });
    if(cncl+ee>0)pcatTotals[pc]=cncl+ee;
  });
  var pcatKeys=Object.keys(pcatTotals).sort(function(a,b){return pcatTotals[b]-pcatTotals[a];});
  charts.pcat=new Chart(document.getElementById("pcatChart"),{type:"doughnut",data:{labels:pcatKeys,datasets:[{data:pcatKeys.map(function(k){return pcatTotals[k];}),backgroundColor:["#388bfd","#f85149","#3fb950","#e3b341","#bc8cff"],borderWidth:0,hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:false,cutout:"62%",plugins:{legend:{position:"right",labels:{color:"#8b949e",font:{size:11},boxWidth:10,padding:8}}}}});
