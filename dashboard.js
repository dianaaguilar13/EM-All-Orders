...
// Get the base tree for current filter selection
function getBaseTree(){
  if(selP.size===1){
    return (D.PT&&D.PT[Array.from(selP)[0]])||{};
  } else if(selP.size>1){
    // Merge trees for multiple partners
    var merged={};
    selP.forEach(function(p){
      var t=(D.PT&&D.PT[p])||{};
      mergeTree(merged,t,4);
    });
    return merged;
  } else if(getPcat()){
    return (D.PCT&&D.PCT[getPcat()])||{};
  }
  return D.GT||{};
}

function mergeTree(dst,src,depth){
  Object.keys(src).forEach(function(k){
    if(depth<=1){dst[k]=(dst[k]||0)+src[k];}
    else{if(!dst[k])dst[k]={};mergeTree(dst[k],src[k],depth-1);}
  });
}

// Count total records in a tree node
function countTree(node,depth){
  if(depth<=0||typeof node!=="object")return node||0;
  var total=0;
  Object.values(node).forEach(function(v){total+=countTree(v,depth-1);});
  return total;
}

// Filter tree by date range (month year format "Jun 2025")
function filterTreeByDate(tree){
  var r=getRange();
  var result={};
  Object.keys(tree).forEach(function(my){
    // Convert "Jun 2025" to "2025-06" for comparison
    try{
      var d=new Date(my);
      var key=(d.getFullYear())+"-"+String(d.getMonth()+1).padStart(2,"0");
      if(key>=r.df&&key<=r.dt)result[my]=tree[my];
    }catch(e){result[my]=tree[my];}
  });
  return result;
}
...
function getSkuBuckets(){
  var pcat=getPcat();
  if(selP.size>0){var merged={};selP.forEach(function(p){var ps=(D.PSKU&&D.PSKU[p])||{};Object.keys(ps).forEach(function(sku){if(!merged[sku])merged[sku]=[0,0,0,0,0,0,0,0];var v=ps[sku];for(var i=0;i<8;i++)merged[sku][i]+=(v[i]||0);});});return merged;}
...
// ── Decomp tree helpers ───────────────────────────────────
// Get level items from tree traversal
function getDecompLevel(levelIdx,prevSelections){
  var tree=filterTreeByDate(getBaseTree());

  // Navigate to current depth based on previous selections
  // Levels: 0=root, 1=active_status(my+act), 2=month_year, 3=refund_month, 4=cncl_status, 5=refund_days
  // Tree structure: my -> act -> cncl -> rm -> rd -> count

  if(levelIdx===0){
    var total=countTree(tree,4);
    return [{l:"Total Units",v:total,c:"#388bfd"}];
  }

  if(levelIdx===1){
    // Active status: sum across all months
    var active=0,inactive=0;
    Object.values(tree).forEach(function(actMap){
      active+=countTree(actMap["Active"]||{},3);
      inactive+=countTree(actMap["Inactive"]||{},3);
    });
    return [{l:"Active",v:active,c:"#58a6ff"},{l:"Inactive",v:inactive,c:"#f85149"}].filter(function(x){return x.v>0;});
  }

  if(levelIdx===2){
    // Month Year - filtered by active status selection
    var selAct=prevSelections[1]===0?"Active":"Inactive";
    var myMap={};
    Object.keys(tree).forEach(function(my){
      var cnt=countTree((tree[my]&&tree[my][selAct])||{},3);
      if(cnt>0)myMap[my]=cnt;
    });
    return Object.entries(myMap).sort(function(a,b){return b[1]-a[1];}).map(function(e){return{l:e[0],v:e[1],c:"#388bfd"};});
  }

  if(levelIdx===3){
    // Refund Month - filtered by active status + month year
    var selAct=prevSelections[1]===0?"Active":"Inactive";
    var myItems=getDecompLevel(2,prevSelections);
    var selMy=myItems[prevSelections[2]]&&myItems[prevSelections[2]].l;
    if(!selMy)return[];
    var actNode=(tree[selMy]&&tree[selMy][selAct])||{};
    // actNode: cncl -> rm -> rd -> count
    var rmMap={};
    Object.values(actNode).forEach(function(rmNode){
      Object.keys(rmNode).forEach(function(rm){
        if(rm==="none")return;
        rmMap[rm]=(rmMap[rm]||0)+countTree(rmNode[rm],1);
      });
    });
    return Object.entries(rmMap).sort(function(a,b){return b[1]-a[1];}).map(function(e){return{l:e[0],v:e[1],c:"#f85149"};});
  }

  if(levelIdx===4){
    // CNCL Status - filtered by active status + month year + refund month
    var selAct=prevSelections[1]===0?"Active":"Inactive";
    var myItems=getDecompLevel(2,prevSelections);
    var selMy=myItems[prevSelections[2]]&&myItems[prevSelections[2]].l;
    var rmItems=getDecompLevel(3,prevSelections);
    var selRm=rmItems[prevSelections[3]]&&rmItems[prevSelections[3]].l;
    if(!selMy||!selRm)return[];
    var actNode=(tree[selMy]&&tree[selMy][selAct])||{};
    var cnclMap={};
    Object.keys(actNode).forEach(function(cncl){
      var cnt=countTree(actNode[cncl][selRm]||{},1);
      if(cnt>0)cnclMap[cncl]=cnt;
    });
    var colors={"Cancelled":"#f85149","Entry Error":"#e3b341","Upgrade":"#3fb950","Downgrade":"#bc8cff","Switch":"#58a6ff","Sale":"#39d353"};
    return Object.entries(cnclMap).sort(function(a,b){return b[1]-a[1];}).map(function(e){return{l:e[0],v:e[1],c:colors[e[0]]||"#388bfd"};});
  }

  if(levelIdx===5){
    // Refund Days - filtered by all previous
    var selAct=prevSelections[1]===0?"Active":"Inactive";
    var myItems=getDecompLevel(2,prevSelections);
    var selMy=myItems[prevSelections[2]]&&myItems[prevSelections[2]].l;
    var rmItems=getDecompLevel(3,prevSelections);
    var selRm=rmItems[prevSelections[3]]&&rmItems[prevSelections[3]].l;
    var cnclItems=getDecompLevel(4,prevSelections);
    var selCncl=cnclItems[prevSelections[4]]&&cnclItems[prevSelections[4]].l;
    if(!selMy||!selRm||!selCncl)return[];
    var actNode=(tree[selMy]&&tree[selMy][selAct])||{};
    var rdNode=(actNode[selCncl]&&actNode[selCncl][selRm])||{};
    var rdLabels={"0":"Same day","15":"<=15 days","30":"<=30 days","45":"<=45 days","60":"<=60 days","61":"61+ days","none":"No date"};
    return Object.entries(rdNode).filter(function(e){return e[0]!=="none"&&e[1]>0;}).sort(function(a,b){return +a[0]-+b[0];}).map(function(e){return{l:rdLabels[e[0]]||e[0],v:e[1],c:"#388bfd"};});
  }
  return[];
}
...
function renderDecompLevels(){
  ...
  for(var li=0;li<=showUpTo;li++){
    var items=getDecompLevel(li,treePath);
    if(!items||items.length===0)break;
    var selIdx=(li>0&&treePath[li-1]!==undefined)?treePath[li-1]:-1;
    var isClickable=(li===showUpTo&&li<5);
    var totalAtLevel=items.reduce(function(s,x){return s+x.v;},0);
    ...
    var hdr=document.createElement("div");hdr.style.cssText="font-size:10px;font-weight:600;color:#8b949e;text-transform:uppercase;letter-spacing:.6px;text-align:center;padding:5px 8px 8px;border-bottom:1px solid #30363d;margin-bottom:6px";hdr.textContent=LEVEL_TITLES[li];col.appendChild(hdr);
    ...
    for(var idx=0;idx<items.length;idx++){
      (function(item,capturedIdx,capturedLi){
        ...
        var pct=totalAtLevel>0?(item.v/totalAtLevel*100).toFixed(1):"0";
...
function buildDecompBC(){
  ...
  treePath.forEach(function(selIdx,i){
    sep();
    var items=getDecompLevel(i+1,treePath.slice(0,i));
    var item=items&&items[selIdx];
    if(item){(function(ci){crumb(item.l,function(){treePath=treePath.slice(0,ci+1);renderDecompLevels();buildDecompBC();},i===treePath.length-1);})(i);}
  });
  if(treePath.length>0){
    ...
    var rst=document.createElement("span");rst.style.cssText="font-size:11px;color:#f85149;cursor:pointer;padding:2px 8px;border-radius:4px;border:1px solid #f8514933;background:#f8514911";rst.textContent="Reset";rst.onclick=function(){treePath=[];renderDecompLevels();buildDecompBC();};bc.appendChild(rst);
  }
}
